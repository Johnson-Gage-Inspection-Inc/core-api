...
def test_get_access_token_success(mock_exists, mock_load_dotenv, mock_pca, mock_cache_class, mock_file, monkeypatch):
...
@mock.patch("dotenv.load_dotenv")
@mock.patch("os.path.exists")
def test_get_access_token_error(mock_exists, mock_load_dotenv, mock_pca, mock_cache_class, mock_file, monkeypatch):
    monkeypatch.setenv("SKIP_AUTH", "false")
    monkeypatch.setenv("AZURE_CLIENT_ID", "test-client-id")
    monkeypatch.setenv("AZURE_TENANT_ID", "test-tenant-id")
    monkeypatch.setenv("AZURE_API_AUDIENCE", "https://api.example.com")
