...
class TestPyroStandardsSharePointClient:
    ...
    def test_get_pyro_standards_excel_file_search_fallback(self, filename, expected_hash):
        """Test fallback to search when direct path fails."""
        result = get_pyro_standards_excel_file(filename)
        assert list(result.keys()) == ['sheet_names', 'total_sheets', 'sheets', 'file_info']
        result_hash = hashlib.md5(result["file_info"]["name"].encode()).hexdigest()
        assert result_hash == expected_hash, "File name hash does not match expected value."
